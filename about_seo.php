...
if (isset($_REQUEST['seopages_id'])) {
        extract($_REQUEST);
	    $user->get_SEOPagesdata($seopages_id);
}
$id= $_GET['seopages_id'];
if(isset($_REQUEST['submit'])){
  extract($_REQUEST);
              $result = $user->update_SEOPages($id,$url,$title,$description,$keywords);
    if($result){
     ...
     <form method="post" action="" enctype="multipart/form-data">
            ...
            <div class="row">
                 <div class="form-group col-md-12">
                        ...
                        <input type="text" class="form-control" id="inputBasicFirstName" name="url" autocomplete="off" value="<?php echo $user->seourl; ?>" required>
                </div>
                <div class="form-group col-md-12">
                        <label class="form-control-label" for="inputBasicFirstName"> Title</label>
                         <textarea id="summernote" data-plugin="summernote"   name="title" required><?php echo $user->seotitle; ?></textarea>
                       
                </div>
                <div class="form-group col-md-12">
                        <label class="form-control-label" for="inputBasicFirstName">Description</label>
                       <textarea id="summernote" data-plugin="summernote"   name="description" required><?php echo $user->seodescription; ?></textarea>
                </div>
               <div class="form-group col-md-12">
                        <label class="form-control-label" for="inputBasicFirstName">Keywords</label>
                    
                       <textarea id="summernote" data-plugin="summernote"   name="keywords"  required><?php echo $user->seokeywords; ?></textarea>
                       
                </div>
               
                    
                <div class="form-group col-md-12 text-center">
                      <button type="submit" name="submit" class="btn btn-primary">Save SEO Pages</button>
                </div>
